  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        ...
        const allEndpoint = "http://empolyee-backedn.onrender.com/api/reports/all";
        const userEndpoint = "http://empolyee-backedn.onrender.com/api/reports";
        ...
        if (err.response?.status === 403 && canModify) {
          try {
            const token = localStorage.getItem("token_real");
            const fallbackResponse = await axios.get("http://empolyee-backedn.onrender.com/api/reports", {
              headers: { "x-auth-token": token || "" },
            });

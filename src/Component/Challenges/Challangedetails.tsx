...
const BASE_URL = "http://localhost:5000"; // Fixed trailing space
...
const Banner = () => {
  ...
  const fetchChallenges = async () => {
    try {
      ...
      message.error("Failed to fetch challenges. Please try again later.");
    }
  };
  ...
  const handleParticipate = async (challengeId: string) => {
    try {
      ...
      const response = await axios.post(`${BASE_URL}/api/participate/${challengeId}`, {
        participantId,
      });
      ...
      {bannerData.map((banner: any) => (
        ...
        >
          <div className="flex max-lg:flex-col items-center">
            <div className="flex-shrink-0 mr-4 max-lg:mb-4">
              <img
                src='/challange.png'
                alt="Banner"
          ...
          <div className="flex-shrink-0 flex flex-col items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <img src="/dot1.png" alt="Points icon" className="w-7 h-7" />
              <div className="text-[#FA541C] flex gap-1">
